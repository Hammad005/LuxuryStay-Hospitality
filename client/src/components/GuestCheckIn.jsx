...
const GuestCheckIn = () => {
  ...
  const {createDirectCheckIn, checkInLoading, resetSuccess} = checkInStore();
  const uniqueRooms = [
    ...new Map(
  ...
  useEffect(() => {
    ...
    let nights = 0;
    if (data.checkOut) {
      const inDate = new Date().setHours(0, 0, 0, 0);
  ...
  }, [data.roomsId, data.checkOut, rooms]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      const response = await createDirectCheckIn(data);
      if (response.success) {
        resetSuccess();
      ...
      <div className="p-4 md:p-16 flex flex-col min-h-screen z-10">
        ...
        >
          ...
          <div className="md:mt-12 mt-6 flex justify-center">
            ...
            >
              {
                checkInLoading ? <Loader2 className="animate-spin"/> : "Check In"
              }
            </Button>
          </div>
        </motion.form>
